...
export async function generateMetadata({
  ...
  try {
    ...
    const baseMetadata = {
      title: challengerWizard
        ? `Challenge from ${challengerWizard.name}`
        : "Join Duel",
    ...
    if (convexUrl) {
      ...
      if (duel.featuredIllustration) {
        ...
        altText = challengerWizard
          ? `Challenge from ${challengerWizard.name}`
          : "Magical duel challenge";
...
export default function JoinShortcodeLayout({
  children,
}: JoinShortcodeLayoutProps) {
  return children;
}
