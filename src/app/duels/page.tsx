  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-100">
      ...
      <main className="container mx-auto px-6 py-12">
        <div className="mb-8">
          <h2 className="text-3xl font-bold text-gray-900 mb-2">Magical Duels</h2>
          <p className="text-gray-600">Join existing duels or create your own magical battles</p>
        </div>
          ...
          <Card>
            ...
            <CardContent>
              <Link href="/duels/join">
                <Button variant="outline" className="w-full">Join Duel</Button>
              </Link>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div>
            <h3 className="text-2xl font-bold text-gray-900 mb-4">Your Duels</h3>
            {playerDuels === undefined ? (
              ...
              <Card>
                <CardContent className="text-center py-12">
                  ...
                  <p className="text-gray-600 mb-4">Create or join a duel to start your magical battles.</p>
                ...
                {playerDuels.map((duel) => (
                  <Card key={duel._id}>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Round {duel.currentRound} • {duel.wizards.length} wizards • Created {new Date(duel.createdAt).toLocaleDateString()}
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
          ...
          <div>
            <h3 className="text-2xl font-bold text-gray-900 mb-4">Available Duels</h3>
            {activeDuels === undefined ? (
              <div className="text-center py-8">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-purple-600 mx-auto"></div>
                <p className="text-gray-500 mt-2">Loading available duels...</p>
              </div>
            ) : activeDuels.filter(duel => !duel.players.includes(user?.id || "")).length === 0 ? (
              <Card>
                <CardContent className="text-center py-12">
                  <div className="text-6xl mb-4">🔍</div>
                  <h3 className="text-xl font-semibold mb-2">No available duels</h3>
                  <p className="text-gray-600 mb-4">Be the first to create a new duel!</p>
                  <Link href="/duels/create">
                  ...
                  .map((duel) => (
                    <Card key={duel._id}>
                      <CardHeader>
                        ...
                        <CardDescription>
                          {duel.wizards.length} wizards • Created {new Date(duel.createdAt).toLocaleDateString()}
                          {duel.shortcode && duel.status === "WAITING_FOR_PLAYERS" && (
                            <span className="ml-2">
                              • Code: <code className="px-1 py-0.5 bg-purple-100 text-purple-800 rounded text-xs">{duel.shortcode}</code>
                            </span>
                          )}
                        </CardDescription>
                      </CardHeader>
