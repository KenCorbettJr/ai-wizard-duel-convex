...
function ActiveDuelsCard({ userId }: { userId?: string }) {
  const playerDuels = useQuery(api.duels.getPlayerDuels, 
    userId ? { userId } : "skip"
  )

  const activeDuels = playerDuels?.filter(duel => 
        ...
        {activeDuels.length === 0 ? (
          <div className="text-center py-4">
            ...
            <Link href="/duels/create">
              <Button variant="outline" size="sm">Create Duel</Button>
            </Link>
          </div>
        ) : (
          <div className="space-y-3">
            {activeDuels.slice(0, 3).map((duel) => (
              <div key={duel._id} className="flex items-center justify-between p-2 border rounded">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <Badge variant={duel.status === "WAITING_FOR_PLAYERS" ? "secondary" : "default"}>
                  ...
                  <p className="text-sm text-gray-600">
                    {typeof duel.numberOfRounds === 'number' 
                      ...
                      : 'To the death'
                    } • {duel.players.length} player{duel.players.length !== 1 ? 's' : ''}
                  </p>
                </div>
            ...
            {activeDuels.length > 3 && (
              <div className="text-center pt-2">
                <Link href="/duels">
                  <Button variant="outline" size="sm">View All ({activeDuels.length})</Button>
                </Link>
              </div>
            )}
          </div>
...
export default function Dashboard() {
  ...
  const totalWins = wizards?.reduce((sum, wizard) => sum + (wizard.wins || 0), 0) || 0
  const totalLosses = wizards?.reduce((sum, wizard) => sum + (wizard.losses || 0), 0) || 0
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-100">
      ...
      <main className="container mx-auto px-6 py-12">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          ...
          <Card>
            ...
            <CardContent>
              <Link href="/duels/join">
                <Button variant="outline" className="w-full">Join Existing Duel</Button>
              </Link>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Create a new wizard for battle
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Button 
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex justify-between">
                  <span className="text-gray-600">Total Wizards:</span>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-green-600">{totalWins}</span>
                </div>
                <div className="flex justify-between">
            ...
            <CardContent>
              {totalBattles === 0 ? (
                ...
                <div className="space-y-3">
                  ...
                  <Link href="/duels">
                    <Button variant="outline" className="w-full">View All Duels</Button>
                  </Link>
                </div>
              )}
            </CardContent>
        ...
        <div className="mb-8">
          <h3 className="text-2xl font-bold text-gray-900 mb-4">Your Wizards</h3>
          {wizards === undefined ? (
            <div className="text-center py-8">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-purple-600 mx-auto"></div>
              <p className="text-gray-500 mt-2">Loading wizards...</p>
            </div>
          ) : wizards.length === 0 ? (
            <Card>
              <CardContent className="text-center py-12">
                ...
                <p className="text-gray-600 mb-4">Create your first wizard to start your magical journey.</p>
